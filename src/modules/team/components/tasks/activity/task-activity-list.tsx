...
import { Clock, Send, Edit2, Trash2, CalendarDays, User, Plus, Smile } from "lucide-react";
...
export function TaskActivityList({
  ...
  // Format date helper: "Jul 31 · 10:14 AM"
  ...
  return (
    <div className="space-y-4 text-left">
      ...
      {activeTab === "worklog" && (
        <div className="space-y-4">
          {/* Work log tracking summary banner */}
              ...
              {workLogs.map((log) => (
                <div key={log.id} className="flex gap-3 items-start text-xs">
                  ...
                  <div className="flex-1 min-w-0 bg-white border border-gray-150 rounded-xl p-3.5 shadow-sm text-left">
                    ...
                    {log.description && (
                      <p className="text-gray-600 font-medium mt-1.5 text-[11px] pl-1.5 border-l-2 border-gray-150 italic leading-snug">
                        {log.description}
                      </p>
                    )}
                  </div>
